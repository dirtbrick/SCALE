...
func match_anim(anim,node):
	match anim:
		...
		RSQUISH:
			...
			tween.tween_property(node, "rect_scale", Vector2(0.8,0.8),0.15)
			#tween.parallel().tween_property(node, 'rect_position',Vector2(default.x+pixel_diff.x/2,default.y+pixel_diff.y/2),0.15)
		SQUISH:
			tween.set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_OUT)
			node.rect_scale = Vector2(0.8,0.8)
			#node.rect_position = Vector2(default.x+pixel_diff.x/2,default.y+pixel_diff.y/2)
			tween.tween_property(node, "rect_scale", Vector2.ONE,0.15)
			#tween.parallel().tween_property(node, 'rect_position', Vector2(default.x,default.y),0.15)
		POPOUT:
			tween.set_trans(Tween.TRANS_ELASTIC).set_ease(Tween.EASE_OUT)
